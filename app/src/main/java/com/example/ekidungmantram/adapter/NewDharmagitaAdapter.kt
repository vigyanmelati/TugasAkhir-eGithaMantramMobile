...
import com.example.ekidungmantram.model.NewDharmagitaModel

class NewDharmagitaAdapter (val results: ArrayList<NewDharmagitaModel.Data>, val listener: OnAdapterListener)
    : RecyclerView.Adapter<NewDharmagitaAdapter.ViewHolder>() {
...
//        Glide.with(holder.view).load(Constant.IMAGE_URL+result.gambar).into(holder.gambar)
    ...
    fun setData (data: List<NewDharmagitaModel.Data>) {
        results.clear()
        results.addAll(data)
    ...
    interface OnAdapterListener{
        fun onClick(result: NewDharmagitaModel.Data)
    }
}